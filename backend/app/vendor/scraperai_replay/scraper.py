"""The replay engine (vendored from ``scraperai/scraper.py``).

Given a ``ScraperConfig`` (produced offline by the real ``scraperai`` build
tool) and a ``BaseCrawler``, ``Scraper.scrape()`` yields one ``dict`` per item.
Imports rewritten to be relative to this vendored package.  See ``__init__.py``
...
from .field_utils import extract_fields_from_html, extract_fields_from_tree
from .models import ScraperConfig, WebpageType
from .urls import fix_relative_url

logger = logging.getLogger('scraperai_replay')
